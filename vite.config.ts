...
import { metaTagsPlugin } from './vite-plugin-meta-tags'
...
export default defineConfig({
  plugins: [
    ...
    tailwindcss(),
    metaTagsPlugin(),
    // Copy PHP meta handler to dist during build
    {
      name: 'copy-meta-handler',
      closeBundle() {
        const destPath = path.resolve(__dirname, 'dist/meta-handler.php');
        
        // Read the production version content with correct path
        const productionContent = `<?php
// Get the request URI
...
?>`;
        ...
        fs.writeFileSync(destPath, productionContent);
        console.log('✓ Copied meta-handler.php to dist');
      }
    }
  ],
  resolve: {
